...
def load_uvfits(file_name, polarization='stokes'):
    ...
    if obs.polrep == "circ":
        ...
        for kk in ["rr","rl", "lr", "ll"]:
            ...
            sigma.append(np.nan_to_num(obs.data[f"{kk}sigma"].reshape(-1, 1), nan=np.inf))
        vis = np.array(vis)
    ...
    uvw = np.vstack([obs.data["u"], obs.data["v"], 0.*obs.data["u"]]).T
    uvw *= rve.SPEEDOFLIGHT/obs.rf
    antpos = rve.AntennaPositions(uvw, ant1, ant2, time)
    return rve.Observation(antpos, vis, 1/sigma/sigma, pol, freq, auxiliary_tables={"ANTENNA": anttbl})
